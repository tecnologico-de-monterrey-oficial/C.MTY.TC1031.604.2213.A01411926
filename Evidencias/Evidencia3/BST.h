...
public:
    ...
    void searchByUbi(string ubisearch);
};
...
// Creamos uan función parecida a find que nos despliega el valor encontrado con la ubi
// Si no lo encuentra despliega "NO EXISTE UN DATO CON ESA UBI"
template<class T>
void BST<T>::searchByUbi(string ubisearch){
    NodeT<T>* aux = root;
    while (aux != nullptr) {
        if (aux->data.ubi == ubisearch) {
            cout<< "Entrada: " << aux->data.entry << "UBI: " << aux->data.ubi << " datetime: " << aux->data.date << " " << aux->data.time << endl;
            return;
        } else {
            if (ubisearch < aux->data.ubi) {
                aux = aux->left;
            } else {
                aux = aux->right;
            }   
    ...
    cout<<"NO EXISTE UN DATO CON ESA UBI"<<endl;
    return;
}
#endif